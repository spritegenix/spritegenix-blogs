...
class CaseStudy extends BaseController
{
    public function index()
    {
        ...
        $builder->where('status', 'published')->where('company_id', COMPANY_ID)->where('post_name', 'caseStudy')->orderBy('datetime', 'DESC');
        $sql = $builder->getCompiledSelect();
       

        $totalPosts = $PostsModel->where('status', 'published')->where('company_id', COMPANY_ID)->where('post_name', 'caseStudy')->countAllResults();
        $totalPages = ceil($totalPosts / $perPage);

        $posts = $PostsModel->where('status', 'published')->where('company_id', COMPANY_ID)->where('post_name', 'caseStudy')->orderBy('datetime', 'DESC')->limit($perPage, $offset)->findAll();

        // Fetch latest 4 posts for the sidebar
        $latestPosts = $PostsModel->where('status', 'published')
                                  ->where('company_id', COMPANY_ID)
                                  ->where('post_name', 'caseStudy')
                                  ->orderBy('datetime', 'DESC')
                                  ->limit(4)
                                  ->findAll();

        // Fetch categories that have case study posts
        $categories = $PostCategoryModel
            ...
            ->where('posts.post_name', 'caseStudy')
            ->distinct()
            ->orderBy('post_categories.category_name', 'ASC')
            ->findAll();

        $data = [
            'title' => 'Case Study - SpriteGenix',
            'posts' => $posts,
    ...
    public function details($slug = null)
    {
        ...
        $post = $PostsModel->where('slug', $slug)->where('status', 'published')->where('company_id', COMPANY_ID)->where('post_name', 'caseStudy')->first();
        ...
        $data = [
            'title' => $post['title'] . ' - SpriteGenix Case Study',
         ...
         echo view('layout/header', $data);
        echo view('case-study-single', $data); // Changed to case-study-single
        echo view('layout/footer', $data);
    }
